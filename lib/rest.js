...
module.exports = function (app, config) {
    ...
    */
    app.get(urlPrefix + '/:db/:collection/:id?', function(req, res) {
        ...
        var datatypes = req.query.datatypes || null;
        datatypes = JSON.parse(datatypes);

        for( t in datatypes){
          console.log(query[t])
          if(datatypes[t] == 'ObjectId'){
            query[t] = new BSON.ObjectID(query[t])
          }
        }
        connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
    ...
    */
    app.post(urlPrefix + '/:db/:collection', function(req, res) {
        if(req.body) {
          ...
          for( t in datatypes){
            if(datatypes[t] == 'ObjectId'){
              body[t] = new BSON.ObjectID(req.body[t])
            }
          }
            connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
                ...
                db.collection(req.params.collection, function(err, collection) {
                    ...
                    collection.insert(
                        ...
                        function(err, docs) {
                            ...
                            res.status(201).json({ _id: docs[0]._id.toHexString(), ok: 1 });
                        }
                    );
                });
            });
        } else {
            res.status(500).json({ message: 'No request body sent.' });
        }
    });
    ...
    app.put(urlPrefix + '/:db/:collection/:id', function(req, res) {
        ...
        connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
            ...
            db.collection(req.params.collection, function(err, collection) {
                ...
                collection.update(spec, body, true, function(err, docs) {
                    res.json({ ok: 1 });
                });
            });
        });
